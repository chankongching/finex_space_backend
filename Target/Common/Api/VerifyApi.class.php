...
class VerifyApi{
    ...
    public  $cliectRedis=NULL;   //保存redis 的session类
    /**
     * 架构方法 设置参数
     * @access public     
    ...
    public function __construct($config=array()){
    	...
    	$this->cliectRedis=RedisIndex::getInstance(); //实例化了该类 已经开启了
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
        //$secode = session($key); //修改此机制
       $secode=$this->cliectRedis->getSessionValue($key); //获取seesion
        if(empty($code) || empty($secode)) 
        {
            return false;
        }
        // session 过期
        //echo NOW_TIME-$secode['verify_time'].'----'.$this->expire;die;
        if(NOW_TIME - $secode['verify_time'] > $this->expire) {
           // session($key, null);
            $this->cliectRedis->delSessionRedis($key);  //清除seesion
            return false;
        }
        //echo $this->authcode(strtoupper($code)).'-------'.$secode['verify_code'];die;
        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            //$this->reset && session($key, null); //删除session
            $this->reset && $this->cliectRedis->delSessionRedis($key);
            return true;
        }
...
//         session($key.$id, $secode);
       ...
       $this->cliectRedis->setSessionRedis($key.$id, $secode);
